...
describe('Notification i18n', () => {
    describe('task_due_soon', () => {
        ...
        it('should handle < 1 hour in EN and PT', () => {
            ...
            expect(resEn.message).toBe('Your task "Test Task" is due in less than 1 hour');

            const resPt = t('task_due_soon', 'pt', { ...p, hoursUntilDue: 0 });
            expect(resPt.message).toBe('Sua tarefa "Test Task" vence em menos de 1 hora');
        });

        it('should handle 1 hour pluralization correctly in EN and PT', () => {
            const resEn = t('task_due_soon', 'en', { ...p, hoursUntilDue: 1 });
            expect(resEn.message).toBe('Your task "Test Task" is due in 1 hour');

            const resPt = t('task_due_soon', 'pt', { ...p, hoursUntilDue: 1 });
            expect(resPt.message).toBe('Sua tarefa "Test Task" vence em 1 hora');
        });

        it('should handle > 1 hour pluralization correctly in EN and PT', () => {
            const resEn = t('task_due_soon', 'en', { ...p, hoursUntilDue: 5 });
            expect(resEn.message).toBe('Your task "Test Task" is due in 5 hours');

            const resPt = t('task_due_soon', 'pt', { ...p, hoursUntilDue: 5 });
            expect(resPt.message).toBe('Sua tarefa "Test Task" vence em 5 horas');
        });
    ...
    describe('task_overdue', () => {
        ...
        it('should handle 1 day overdue in EN and PT', () => {
            ...
            expect(resEn.message).toBe('Your task "Test Task" was due yesterday');
        ...
        it('should handle N days overdue in EN and PT', () => {
            ...
            expect(resEn.message).toBe('Your task "Test Task" was due 5 days ago');

            const resPt = t('task_overdue', 'pt', { ...p, daysOverdue: 5 });
            expect(resPt.message).toBe('Sua tarefa "Test Task" vencia há 5 dias');
        });
    });
});
