...
| [05b-medium-effort.md](05b-medium-effort.md) | Médio — 1 sessão focada por item | Eliminar `PRAGMA foreign_keys=OFF` global, side effects fora de transação, CASCADEs reais, error handling do tasks, destino do BaseRepository, renomear locales `jp`/`ua` |
...
storage (absorvida por [06-docs-update.md](06-docs-update.md)).

Ordem sugerida quando forem executados: 05a inteiro → ME-1 → ME-2 → ME-3 →
demais de 05b → 05c. Prioridade de risco mais alta: ME-1 (integridade de FKs)
e HE-1 (módulo tasks). Regras de execução por agente: [README.md](README.md).
